...
'''
Script originally created by Christopher Laumer to select longest ORF from TransDecoder output. Worked with older versions of Trinity output, such as 20140413.
This script has been updated by Rosa Fernandez (http://github.com/metazomics/scripts) on March 2021 to work with the current version of Trinity (2.11) and TransDecoder.
Usage:
    In CESGA, load python 3 and biopython 1.62: module load python/3.7.9 biopython/1.62-python-3.7.9
    python3 fetch_longest_iso.py -i file.fasta -o output_longest.fasta -t -l
'''
...
def sorted_headers(trans_dict):
    ...
    '''
    headers = list(trans_dict.keys())
    transcript_id = re.compile('\d*_DN\d*_c\d*_g\d*\_i\d*.p\d*')  # Added DN\d*_ for new Trinity gene hierarchy
...
def split_by_isoform(dict, L):
    ...
    '''
    subcomp_id = re.compile('\d*_DN\d*_c\d*_g\d*')  # Added DN\d*_ for new Trinity gene hierarchy
    transcript_id = re.compile('\d_*DN\d*_c\d*_g\d*\_i\d*.p\d*')  # Added DN\d*_ for new Trinity gene hierarchy
    subcomponent = {}
    longest = {}
    P = []
